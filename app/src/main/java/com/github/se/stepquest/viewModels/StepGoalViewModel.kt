...
data class StepGoalsState(
    val newDailyStepGoal: String = "",
    val newWeeklyStepGoal: String = ""
)

class StepGoalsViewModel : ViewModel() {
    private val _state = MutableStateFlow(StepGoalsState())
    val state: StateFlow<StepGoalsState> get() = _state
    ...
    fun calculateAndConfirmGoals(onConfirm: (dailyStepGoal: Int, weeklyStepGoal: Int) -> Unit, onDismiss: () -> Unit) {
        viewModelScope.launch {
            val (dailyStep, weeklyStep) = calculateStepGoals(_state.value.newDailyStepGoal, _state.value.newWeeklyStepGoal)
            onConfirm(dailyStep, weeklyStep)
            onDismiss()
        }
    }

    private fun calculateStepGoals(newDailyStepGoal: String, newWeeklyStepGoal: String): Pair<Int, Int> {
        val dailyStep =
            newDailyStepGoal
                .filter { it.isDigit() }
                .take(5)
